...
const compareOps_1 = require("./compareOps");
...
class Compiler extends obj_1.Obj {
    constructor(templateName, throwOnUndefined) {
        super();
        this.compileNunjucksNodeList = this.compileNodeList;
        this.compileNunjucksSymbol = this.compileSymbol; // Alias for TypeScript project renaming of Symbol -> NunjucksSymbol
        this.compileArrayNode = this.compileArray; // Alias for TypeScript project renaming of Array -> ArrayNode
    ...
    fail(msg, lineno, colno) {
        ...
        throw new templateError_1.TemplateError(msg, lineno, colno);
    }
    _pushBuffer() {
    ...
    _compileExpression(node, frame) {
        ...
        // not.
        this.assertType(node, literal_1.Literal, nunjucksSymbol_1.NunjucksSymbol, group_1.Group, Array, dict_1.Dict, funCall_1.FunCall, caller_1.Caller, filter_1.Filter, lookupVal_1.LookupVal, compare_1.Compare, inlineIf_1.InlineIf, in_1.In, is_1.Is, and_1.And, or_1.Or, not_1.Not, add_1.Add, concat_1.Concat, sub_1.Sub, mul_1.Mul, div_1.Div, floorDiv_1.FloorDiv, mod_1.Mod, pow_1.Pow, neg_1.Neg, pos_1.Pos, compare_1.Compare, nunjucksNode_1.NunjucksNodeList);
    ...
    compileCallExtension(node, frame, async) {
        ...
        if (args) {
            if (!(args instanceof nunjucksNode_1.NunjucksNodeList)) {
    ...
    compilePair(node, frame) {
        ...
        if (key instanceof nunjucksSymbol_1.NunjucksSymbol) {
            key = new literal_1.Literal(key.value, key.lineno, key.colno);
        }
        else if (!(key instanceof literal_1.Literal &&
    ...
    compileCompare(node, frame) {
        ...
        node.ops.forEach((op) => {
            this._emit(` ${compareOps_1.compareOps[op.type]} `);
    ...
    compileFilter(node, frame) {
        ...
        this.assertType(name, nunjucksSymbol_1.NunjucksSymbol);
    ...
    compileFor(node, frame) {
        ...
        // appropriately
        if (node.name instanceof arrayNode_1.ArrayNode) {
    ...
    _compileMacro(node, frame) {
        ...
        node.args.children.forEach((arg, i) => {
            if (i === node.args.children.length - 1 && arg instanceof dict_1.Dict) {
                kwargs = arg;
            }
            else {
                this.assertType(arg, nunjucksSymbol_1.NunjucksSymbol);
                args.push(arg);
            }
        });
    ...
    compileFromImport(node, frame) {
        ...
        node.names.children.forEach((nameNode) => {
            ...
            let alias;
            const id = this._tmpid();
            if (nameNode instanceof pair_1.Pair) {
    ...
    compileOutput(node, frame) {
        ...
        children.forEach((child) => {
            ...
            if (child instanceof templateData_1.TemplateData) {
                if (child.value) {
    ...
    compileRoot(node, frame) {
        ...
        this.inBlock = true;
        const blockNames = [];
        const blocks = node.findAll(block_1.Block);
        blocks.forEach((block, i) => {
            ...
            const tmpFrame = new frame_1.Frame();
...
function compile(src, asyncFilters, extensions, name, opts = { throwOnUndefined: undefined }) {
    const compiler = new Compiler(name, opts.throwOnUndefined);
    ...
    compiler.compile(transformer_1.transform(parser_1.parse(processedSrc, extensions, opts), asyncFilters, name));
