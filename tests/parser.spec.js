(function () {
    ...
    function toNodes(ast) {
        ...
        if (dummy instanceof NunjucksNodeList) {
            ...
            return createInstanceOfType(Type, ast[1], ast[2], ast[3] ? toNodes(ast[3]) : ast[3], lib.isArray(ast[4]) ? lib.map(ast[4], toNodes) : ast[4]);
        }
        else {
            return createInstanceOfType(Type, 0, 0, toNodes(ast[1]), toNodes(ast[2]), toNodes(ast[3]), toNodes(ast[4]), toNodes(ast[5]));
        }
    }
    describe('parser', function () {
        it('should parse basic types', function () {
            isAST(parser.parse('{{ 1 }}'), [Root,
                [Output,
                    [Literal, 1]]]);
            isAST(parser.parse('{{ 4.567 }}'), [Root,
                [Output,
                    [Literal, 4.567]]]);
            isAST(parser.parse('{{ "foo" }}'), [Root,
                [Output,
                    [Literal, 'foo']]]);
            isAST(parser.parse('{{ \'foo\' }}'), [Root,
                [Output,
                    [Literal, 'foo']]]);
            isAST(parser.parse('{{ true }}'), [Root,
                [Output,
                    [Literal, true]]]);
            isAST(parser.parse('{{ false }}'), [Root,
                [Output,
                    [Literal, false]]]);
            isAST(parser.parse('{{ none }}'), [Root,
                [Output,
                    [Literal, null]]]);
            isAST(parser.parse('{{ foo }}'), [Root,
                [Output,
                    [NunjucksSymbol, 'foo']]]);
            isAST(parser.parse('{{ r/23/gi }}'), [Root,
                [Output,
                    [Literal, new RegExp('23', 'gi')]]]);
        });
        it('should parse aggregate types', function () {
            isAST(parser.parse('{{ [1,2,3] }}'), [Root,
                [Output,
                    [ArrayNode,
                        [Literal, 1],
                        [Literal, 2],
            ...
            isAST(parser.parse('{{ (1,2,3) }}'), [Root,
                [Output,
                    [Group,
                        ...
                        [Literal, 2],
                        [Literal, 3]]]]);
            isAST(parser.parse('{{ {foo: 1, \'two\': 2} }}'), [Root,
                [Output,
                    [Dict,
                        [Pair,
                            [NunjucksSymbol, 'foo'],
        ...
        it('should parse variables', function () {
            isAST(parser.parse('hello {{ foo }}, how are you'), [Root,
                [Output, [TemplateData, 'hello ']],
                [Output, [NunjucksSymbol, 'foo']],
                [Output, [TemplateData, ', how are you']]]);
        });
        it('should parse operators', function () {
            isAST(parser.parse('{{ x == y }}'), [Root,
                [Output,
                    [Compare,
                        [NunjucksSymbol, 'x'],
                        [[CompareOperand, [NunjucksSymbol, 'y'], '==']]]]]);
            isAST(parser.parse('{{ x or y }}'), [Root,
                [Output,
                    [Or,
                        [NunjucksSymbol, 'x'],
                        [NunjucksSymbol, 'y']]]]);
            isAST(parser.parse('{{ x in y }}'), [Root,
                [Output,
                    [In,
                        [NunjucksSymbol, 'x'],
                        [NunjucksSymbol, 'y']]]]);
            isAST(parser.parse('{{ x not in y }}'), [Root,
                [Output,
                    [Not,
                        [In,
                            [NunjucksSymbol, 'x'],
                            [NunjucksSymbol, 'y']]]]]);
            isAST(parser.parse('{{ x is callable }}'), [Root,
                [Output,
                    [Is,
                        [NunjucksSymbol, 'x'],
                        [NunjucksSymbol, 'callable']]]]);
            isAST(parser.parse('{{ x is not callable }}'), [Root,
                [Output,
                    [Not,
                        [Is,
                            [NunjucksSymbol, 'x'],
        ...
        it('should parse operators with correct precedence', function () {
            isAST(parser.parse('{{ x in y and z }}'), [Root,
                [Output,
                    [And,
                        [In,
                            [NunjucksSymbol, 'x'],
                            [NunjucksSymbol, 'y']],
                        [NunjucksSymbol, 'z']]]]);
            isAST(parser.parse('{{ x not in y or z }}'), [Root,
                [Output,
                    [Or,
                        [Not,
                            [In,
                                ...
                                [NunjucksSymbol, 'y']]],
                        [NunjucksSymbol, 'z']]]]);
            isAST(parser.parse('{{ x or y and z }}'), [Root,
                [Output,
                    [Or,
                        [NunjucksSymbol, 'x'],
                        [And,
        ...
        it('should parse for loops', function () {
            isAST(parser.parse('{% for x in [1, 2] %}{{ x }}{% endfor %}'), [Root,
                [For,
                    [ArrayNode,
                        [Literal, 1],
                        [Literal, 2]],
        ...
        it('should parse for loops with else', function () {
            isAST(parser.parse('{% for x in [] %}{{ x }}{% else %}empty{% endfor %}'), [Root,
                [For,
                    [ArrayNode],
                    [NunjucksSymbol, 'x'],
                    [NunjucksNodeList,
        ...
        it('should parse filters', function () {
            isAST(parser.parse('{{ foo | bar }}'), [Root,
                [Output,
                    [Filter,
                        [NunjucksSymbol, 'bar'],
                        [NunjucksNodeList,
                            [NunjucksSymbol, 'foo']]]]]);
            isAST(parser.parse('{{ foo | bar | baz }}'), [Root,
                [Output,
                    [Filter,
                        [NunjucksSymbol, 'baz'],
                        [NunjucksNodeList,
                            [Filter,
                                ...
                                [NunjucksNodeList,
                                    [NunjucksSymbol, 'foo']]]]]]]);
            isAST(parser.parse('{{ foo | bar(3) }}'), [Root,
                [Output,
                    [Filter,
                        [NunjucksSymbol, 'bar'],
                        [NunjucksNodeList,
        ...
        it('should parse verbatim', function () {
            isAST(parser.parse('{% verbatim %}hello {{ {% %} }}{% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, 'hello {{ {% %} }}']]]);
        });
        it('should parse verbatim with broken variables', function () {
            isAST(parser.parse('{% verbatim %}{{ x }{% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, '{{ x }']]]);
        });
        it('should parse verbatim with broken blocks', function () {
            isAST(parser.parse('{% verbatim %}{% if i_am_stupid }Still do your job well{% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, '{% if i_am_stupid }Still do your job well']]]);
        });
        it('should parse verbatim with pure text', function () {
            isAST(parser.parse('{% verbatim %}abc{% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, 'abc']]]);
        });
        it('should parse verbatim with verbatim blocks', function () {
            isAST(parser.parse('{% verbatim %}{% verbatim %}{{ x }{% endverbatim %}{% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, '{% verbatim %}{{ x }{% endverbatim %}']]]);
        });
        it('should parse verbatim with comment blocks', function () {
            isAST(parser.parse('{% verbatim %}{# test {% endverbatim %}'), [Root,
                [Output,
                    [TemplateData, '{# test ']]]);
        });
        it('should parse multiple verbatim blocks', function () {
            isAST(parser.parse('{% verbatim %}{{ var }}{% endverbatim %}{{ var }}{% verbatim %}{{ var }}{% endverbatim %}'), [Root,
                [Output, [TemplateData, '{{ var }}']],
                [Output, [NunjucksSymbol, 'var']],
                [Output, [TemplateData, '{{ var }}']]]);
        });
        ...
        it('should parse keyword and non-keyword arguments', function () {
            isAST(parser.parse('{{ foo("bar", falalalala, baz="foobar") }}'), [Root,
                [Output,
                    [FunCall,
                        [NunjucksSymbol, 'foo'],
                        [NunjucksNodeList,
        ...
        it('should parse imports', function () {
            isAST(parser.parse('{% import "foo/bar.njk" as baz %}'), [Root,
                [Import,
                    [Literal, 'foo/bar.njk'],
                    [NunjucksSymbol, 'baz']]]);
            isAST(parser.parse('{% from "foo/bar.njk" import baz, foobar as foobarbaz %}'), [Root,
                [FromImport,
                    [Literal, 'foo/bar.njk'],
                    [NunjucksNodeList,
                        [NunjucksSymbol, 'baz'],
            ...
            isAST(parser.parse('{% import "foo/bar.html"|replace("html", "j2") as baz %}'), [Root,
                [Import,
                    [Filter,
                    ...
                    [NunjucksSymbol, 'baz']]]);
            isAST(parser.parse('{% from ""|default("foo/bar.njk") import baz, foobar as foobarbaz %}'), [Root,
                [FromImport,
                    [Filter,
        ...
        it('should parse whitespace control', function () {
            ...
            isAST(parser.parse('{% from x import y -%}\n  hi \n'), [Root,
                ...
                [Output,
                    [TemplateData, 'hi \n']]]);
            isAST(parser.parse('{% if x -%}{{y}} {{z}}{% endif %}'), [Root,
                [If,
                    [NunjucksSymbol, 'x'],
                    [NunjucksNodeList,
                        [Output,
                        ...
                        [Output,
                            [NunjucksSymbol, 'z']]]]]);
            isAST(parser.parse('{% if x -%}{% if y %} {{z}}{% endif %}{% endif %}'), [Root,
                [If,
                    [NunjucksSymbol, 'x'],
                    [NunjucksNodeList,
                        [If,
            ...
            isAST(parser.parse('{% if x -%}{# comment #} {{z}}{% endif %}'), [Root,
                [If,
                    [NunjucksSymbol, 'x'],
                    [NunjucksNodeList,
                        [Output,
        ...
        it('should parse custom tags', function () {
            ...
            function TestBlockTagExtension() {
                ...
                this.parse = function (parser, nodes) {
                    ...
                    var tag = new CallExtension(this, 'bar', null, [1, content]);
                    parser.advanceAfterBlockEnd();
                    return tag;
                };
            }
            function TestArgsExtension() {
                ...
                this.parse = function (parser, nodes) {
                    var begun = parser.parserTokenStream.peekToken();
                    var args = null;
                    // Skip the name
            ...
            isAST(parser.parse('{% testtag %}', extensions), [Root,
                [CallExtension, extensions[0], 'foo', undefined, undefined]]);
            isAST(parser.parse('{% testblocktag %}sdfd{% endtestblocktag %}', extensions), [Root,
                [CallExtension, extensions[1], 'bar', null,
                    [1, [NunjucksNodeList,
                            [Output,
                                [TemplateData, 'sdfd']]]]]]);
            isAST(parser.parse('{% testblocktag %}{{ 123 }}{% endtestblocktag %}', extensions), [Root,
                [CallExtension, extensions[1], 'bar', null,
                    [1, [NunjucksNodeList,
                            [Output,
                                [Literal, 123]]]]]]);
            isAST(parser.parse('{% testargs(123, "abc", foo="bar") %}', extensions), [Root,
                    ...
                    [NunjucksNodeList,
                        ...
                        [KeywordArgs,
                            [Pair,
                                [NunjucksSymbol, 'foo'],
                                [Literal, 'bar']]]]]]);
            isAST(parser.parse('{% testargs %}', extensions), [Root,
